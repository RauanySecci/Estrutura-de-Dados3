...
void funcionalidade8(char *a1){
    ...
    if(aux_c->status == '0'){
        ...
        principal_cria_grafo(arquivoBinEntrada); 
...
void funcionalidade9(char *a1){
    ...
    if(aux_c->status == '0'){
        ...
        grafo_transposto(arquivoBinEntrada); 
...
void funcionalidade10(char *a1, int n){
    FILE *arquivoBinEntrada = fopen(a1, "rb"); //definição do arquivoBinEntrada como leitura binaria
    if (arquivoBinEntrada == NULL) {
        printf("Falha no processamento do arquivo.\n"); //verificacao de existencia dos arquivos
        return;
    ...
    if(aux_c->status == '0'){
        ...
        lista_tecnologia(arquivoBinEntrada, n); 
...
void funcionalidade11(char *a1){
    ...
    if(aux_c->status == '0'){
        ...
        funcao_aux_11(arquivoBinEntrada); 
...
// Função para a funcionalidade 12
void funcionalidade12(char *a1, int n) {
    n = n*2;
    FILE *arquivoBinEntrada = fopen(a1, "rb");
    if (arquivoBinEntrada == NULL) {
        printf("Falha no processamento do arquivo.\n");
        return;
    }

    Cabecalho *aux_c = malloc(sizeof(Cabecalho));
    fread(&aux_c->status, 1, 1, arquivoBinEntrada);

    if (aux_c->status == '0') {
        printf("Falha no processamento do arquivo.\n");
    } else {
        // Aloca memória para as consultas
        char **consultas = malloc(n * sizeof(char *));
        for (int i = 0; i < n; i+=2) {
            consultas[i] = malloc(100 * sizeof(char));
            consultas[i+1] = malloc(100 * sizeof(char));
            scan_quote_string(consultas[i]);
            scan_quote_string(consultas[i+1]);
        }

        // Chama a função auxiliar
        funcao_aux_12(arquivoBinEntrada, n, consultas);
