...
void lista_tecnologia(FILE *arquivo, int n){
    ...
    while (n > 0){ 
        ...
        int primeiro = 1; // Flag para controlar o primeiro elemento na lista
        ...
        for (int i = 0; i < grafo->numVertices; i++){
            ...
            while (a != NULL){
                if (strcmp(a->nomeTecnologiaDestino, a1) == 0){
                    // Verifica se é o primeiro elemento na lista
                    if (primeiro) {
                        printf("%s: %s", a->nomeTecnologiaDestino, grafo->lista_vertices[i].nomeTecnologia);
                        primeiro = 0;
                    } else {
                        printf(", %s", grafo->lista_vertices[i].nomeTecnologia);
                    }
                }
                a = a->proximo;
            }
